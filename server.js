...
function wallet(u,reason='local'){const prev=clientBalance.get(u.id);clientBalance.set(u.id,u.balance);if(prev===undefined)return null;const delta=Math.round((u.balance-prev)*100)/100;if(Math.abs(delta)<=0.000001)return null;const id=store.id('rtx'),event={id,_id:id,amount:delta,currency:'FLIPCOINS',reason};broadcast(`wallet:transactions#${u.id}`,event,u.id);return event;}
...
const publishWalletEvent=(user,event)=>{clientBalance.set(user.id,user.balance);broadcast(`wallet:transactions#${user.id}`,event,user.id);return event;};
...
function blackjackExactFromPublic(r,user){if(!r)return {id:'',status:'NOT_STARTED'};const fake={id:r.id,dealer:r.dealer||[],hands:r.hands||[],activeHand:r.activeHand||0,proof:r.proof||null};return blackjackExact(fake,user,!!r.done,false);}
...
function blackjackExact(g,user,done=false,resumed=false){if(!g)return {id:'',status:'NOT_STARTED'};const totalBet=(g.hands||[]).reduce((s,h)=>s+Number(h.bet||0),0),dealerDone=done;const hands=(g.hands||[]).map((h,idx)=>{const score=cardScore(h.cards),isCurrent=idx===g.activeHand&&!done&&!h.stood&&score<21;let status='PLAYING';if(done)status=(h.outcome||'LOSE').toUpperCase();else if(score>21)status='BUSTED';else if(h.stood)status='STAND';const possible=isCurrent?['HIT','STAND',...(h.cards.length===2&&user.balance>=h.bet?['DOUBLE']:[]),...(h.cards.length===2&&h.cards[0]?.rank===h.cards[1]?.rank&&user.balance>=h.bet?['SPLIT']:[])]:[];return {cards:h.cards.map((c,i)=>exactCard(c,true,i)),bet:h.bet,status,possibleActions:possible,bestAction:null,payout:h.payout||0};});return {id:g.id,_id:g.id,status:done?'ENDED':resumed?'RESUMED':'IN_PROGRESS',totalGameBet:totalBet,currency:'FLIPCOINS',dealer:{hand:{cards:(g.dealer||[]).map((c,i)=>exactCard(c,dealerDone||i===0,i))}},player:{hands},insuranceBet:0,isInsuranceOffered:false,isInsuranceTaken:false,payout:done?hands.reduce((s,h)=>s+(h.payout||0),0):0,serverSeedHash:g.proof?.serverSeedHash,clientSeed:g.proof?.clientSeed,nonce:g.proof?.nonce};}
...
async function fetchJsonWithTimeout(url,opts={},ms=6500){
  const ac=new AbortController(),timer=setTimeout(()=>ac.abort(),ms);try{const r=await fetch(url,{...opts,signal:ac.signal,headers:{'User-Agent':'BloxFlip-Local-Recreation/2.0','Accept':'application/json',...(opts.headers||{})}});if(!r.ok)throw new Error(`HTTP ${r.status}`);return await r.json();}finally{clearTimeout(timer)}
}
async function fetchBufferWithTimeout(url,ms=9000){
  const ac=new AbortController(),timer=setTimeout(()=>ac.abort(),ms);try{const r=await fetch(url,{signal:ac.signal,headers:{'User-Agent':'BloxFlip-Local-Recreation/2.0','Accept':'image/png,image/webp,image/jpeg'}});if(!r.ok)throw new Error(`HTTP ${r.status}`);return Buffer.from(await r.arrayBuffer());}finally{clearTimeout(timer)}
}
...
async function resolveRobloxProfile(username){
  const clean=cleanUsername(username);if(clean.length<3)throw new Error('Enter a valid Roblox username');
  const cached=store.findUser(clean);
  if(cached?.robloxId&&cached.avatar?.startsWith('/local-avatars/')){const fp=path.join(PUBLIC,cached.avatar);if(fs.existsSync(fp)&&imageContentType(fs.readFileSync(fp)))return cached;}
  try{
    let row=cached?.robloxId?{id:cached.robloxId,name:cached.robloxUsername||cached.username,displayName:cached.displayName||cached.username}:null;
    if(!row){const lookup=await fetchJsonWithTimeout('https://users.roblox.com/v1/usernames/users',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({usernames:[clean],excludeBannedUsers:false})});row=Array.isArray(lookup?.data)?lookup.data[0]:null;}
    if(!row?.id)throw new Error('Roblox user not found');
    let avatar=cached?.avatar||null;
    const user=store.ensureUser(row.name||clean,{robloxId:row.id,robloxUsername:row.name||clean,displayName:row.displayName||row.name||clean,avatar,avatarUpdatedAt:cached?.avatarUpdatedAt||0});
    try{avatar=await cacheRobloxHeadshot(user)||avatar;}catch{}
    if(avatar&&user.avatar!==avatar){user.avatar=avatar;user.avatarUpdatedAt=Date.now();store.saveSoon();}
    return user;
  }catch(e){
    const offline=store.findUser(clean);if(offline)return offline;
    const err=new Error(e?.name==='AbortError'?'Roblox lookup timed out. Check your internet connection.':(String(e?.message||'').includes('not found')?'Roblox user not found':'First login for this Roblox username requires an internet connection.'));err.status=503;throw err;
  }
}
function chatMessage(user,text,extra={}){const now=Date.now(),id=store.id('chat');return {_id:id,id,userId:user?.id||extra.userId||'local-system',username:user?.username||extra.username||'BloxFlip',displayName:user?.displayName||extra.displayName||extra.username||'BloxFlip',avatar:user?.avatar||extra.avatar||'/logo-small.svg',rank:user?.rank||0,level:10,text:String(text||'').slice(0,180),content:String(text||'').slice(0,180),message:String(text||'').slice(0,180),created:now,timestamp:now,system:false,...extra};}
...
async function api(req,res,url){
  try{
    ...
    if(method==='GET'&&p.startsWith('/api/user/avatar/')){
      ...
      if(ident==='1'){const botAvatar=path.join(PUBLIC,'_next','image (2).webp');if(fs.existsSync(botAvatar)){const buf=fs.readFileSync(botAvatar);res.writeHead(200,{'Content-Type':'image/webp','Cache-Control':'public, max-age=86400, immutable'});res.end(buf);return;}}
    ...
    if(method==='GET'&&p==='/api/local/features')return json(res,200,{features:{
      ...
      gameconfig_blackjack:{enabled:true,min:0.1,max:50000},july4promo:{defaultValue:false},crash_v2_enabled:{defaultValue:false},
    ...
    if(method==='GET'&&p==='/api/realtime/events'){
      ...
      const slide=ensureSlideExact();send('slide:new-round',slide);
    ...
    if(method==='POST'&&p==='/api/local/grant'){const user=requireUser(req),raw=Number(b.amount);if(!Number.isFinite(raw)||raw<1||raw>10000000)throw new Error('Enter an amount from 1 to 10,000,000');const amount=games.round2(raw);store.adjustBalance(user,amount,'FAKE_COIN_GRANT');wallet(user,'grant');return json(res,200,{success:true,balance:user.balance,amount});}
    ...
    if(method==='POST'&&p==='/api/games/cases/roll'){
      const user=requireUser(req),before=user.balance,result=games.openCase(user,{slug:b.slug||b.caseIdentifier||b.id,count:1}),won=result.wins[0]||{};
      ...
      feed();return json(res,200,{success:true,itemWon:won.assetId||won.id||won.name,wonItem:won,case:result.case,payout:result.payout,multiplier:result.multiplier,gameEvents:[deltaEvent(before,user,'cases')]});
    }
    ...
    if(method==='POST'&&p==='/api/games/blackjackv2/action'){
      const user=requireUser(req),before=user.balance,r=games.blackjackAction(user,String(b.action||'').toLowerCase()),g=store.state.activeBlackjack[user.id];let ex=g?blackjackExact(g,user,false):blackjackExactFromPublic(r,user);if(!g)feed();return json(res,200,{success:true,game:ex,...ex,gameEvents:Math.abs(user.balance-before)>1e-9?[deltaEvent(before,user,'blackjack')]:[]});
    }
    if(method==='POST'&&p==='/api/games/blackjackv2/insurance'){const user=requireUser(req),g=store.state.activeBlackjack[user.id];if(!g)return json(res,404,{message:'No active game'});return json(res,200,{success:true,game:blackjackExact(g,user,false),...blackjackExact(g,user,false)});}
    if(method==='DELETE'&&p==='/api/games/blackjackv2/my-game'){const user=requireUser(req),g=store.state.activeBlackjack[user.id];if(g){const refund=(g.hands||[]).reduce((s,h)=>s+Number(h.bet||0),0);delete store.state.activeBlackjack[user.id];store.adjustBalance(user,refund,'BLACKJACK_CANCEL_REFUND');wallet(user,'blackjack');}return json(res,200,{success:true});}
    ...
    if(method==='POST'&&p==='/api/games/upgrader/roll'){const user=requireUser(req),items=itemCatalog(),target=items.find(x=>String(x.id)===String(b.targetAssetId))||items.find(x=>String(x.slug)===String(b.targetAssetId))||items.find(x=>Number(x.value)>=Number(b.bet)*Number(b.multiplier||2))||items[items.length-1],before=user.balance,r=games.upgrader(user,{bet:b.bet,targetValue:target?.value||Number(b.bet)*Number(b.multiplier||2),rangeStart:b.rangeStart});feed();const game={id:r.id,_id:r.id,uuid:r.uuid,roll:r.roll,targetItem:target,isWin:r.win,win:r.win,won:r.win,payout:r.payout,multiplier:r.multiplier,winChance:r.chance,rangeStart:r.rangeStart,instantMode:!!b.instantMode};return json(res,200,{success:true,game,result:game,data:game,gameEvents:[deltaEvent(before,user,'upgrader')]});}
...
server.listen(PORT,HOST,()=>{console.log(`\nBloxFlip MODDED V2.6 is running at http://${HOST==='0.0.0.0'?'localhost':HOST}:${PORT}`);console.log('Press Ctrl+C to stop.\n');});
