...
async function fetchJsonWithTimeout(url,opts={},ms=6500){
  const ac=new AbortController(),timer=setTimeout(()=>ac.abort(),ms);try{const r=await fetch(url,{...opts,signal:ac.signal,headers:{'User-Agent':'BloxFlip-MODDED/1.1','Accept':'application/json',...(opts.headers||{})}});if(!r.ok)throw new Error(`HTTP ${r.status}`);return await r.json();}finally{clearTimeout(timer)}
}
async function fetchBufferWithTimeout(url,ms=9000){
  const ac=new AbortController(),timer=setTimeout(()=>ac.abort(),ms);try{const r=await fetch(url,{signal:ac.signal,headers:{'User-Agent':'BloxFlip-MODDED/1.1','Accept':'image/png,image/webp,image/jpeg'}});if(!r.ok)throw new Error(`HTTP ${r.status}`);return Buffer.from(await r.arrayBuffer());}finally{clearTimeout(timer)}
}
...
server.listen(PORT,HOST,()=>{console.log(`\nBloxFlip MODDED v1.1 is running at http://${HOST==='0.0.0.0'?'localhost':HOST}:${PORT}`);console.log('Press Ctrl+C to stop.\n');});
